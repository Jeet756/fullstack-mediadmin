  ...
  if (typeof obj === "object" && obj !== null) {
    ...
    for (let key in obj) {
      if (key.toLowerCase().includes("password") || key.toLowerCase().includes("otp")) {
        newObj[key] = obj[key]; // skip sensitive
      } else {
...
app.get("/api/gallery", async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `SELECT * FROM gallery ORDER BY created_at ${sort}`
    );
...
if (count >= 50) {
  ...
  await client.query(
    `INSERT INTO gallery (image_url, public_id, caption)
     VALUES ($1,$2,$3)`,
    [image_url, public_id, caption || ""]
  );
...
if (isNaN(id)) {
  return res.status(400).json({ message: "Invalid ID" });
      ...
      const result = await pool.query(
        "SELECT public_id FROM gallery WHERE id=$1",
        [id]
      );
...
app.post("/api/reset-password", resetLimiter, async (req, res) => {
  ...
  try {
    ...
    // ✅ Delete only used OTP (IMPORTANT FIX)
    await client.query(
